...
class AdminController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $data = Yii::$app->db->createCommand("SELECT user_x_roles.user_id from user_x_roles where user_x_roles.user_zone_id IN (SELECT user_zone_id from user_x_roles where user_x_roles.user_id = $userId) GROUP BY user_id")->queryAll();
        print_r($data);exit;*/
        ...
        /*$productamounts = Payments::get_total_amount(1);//products amounts
        $adamounts = Payments::get_total_amount(2);//products amounts
        $amounts['amount_actual'] = $productamounts['amount_actual'] + $adamounts['amount_actual'];
        $amounts['amount_paid'] = $productamounts['amount_paid'] + $adamounts['amount_paid'];*/
      
        $productamounts = Payments::get_total_amount(1);//products amounts        
        $adamounts = Payments::get_total_amount(2);//Ads amounts
        $orderamounts = Payments::get_total_amount(3);//Order amounts
        $quoteamounts = Payments::get_total_amount(4);//Quote amounts
        $amounts['amount_actual'] = $productamounts['amount_actual'] + $adamounts['amount_actual'] + $orderamounts['amount_actual'] + $quoteamounts['amount_actual'];
        $amounts['amount_paid'] = $productamounts['amount_paid'] + $adamounts['amount_paid'] + $orderamounts['amount_paid'] + $quoteamounts['amount_paid']; 
...
/*
	...
	*/
    ...
    public function actionViewuser($id)
    {
        ...
        $ads = Ads::get_ads_by_userid($id);
        $quotes = Getquote::get_quotes_by_userid($id);
        echo $this->render('//admin/users/viewuser', array('user' => $user,'products' => $products,'orders' => $orders,'payments' => $payments,'ads' => $ads,'quotes' => $quotes));
    }
    ...
    public function actionApprovead()
    {
        ...
        Ads::post_advt_on_facebook_wall($id); 
    ...
    public function actionPayments(){
        ...
        $product_payment_details = Payments::select_all_payments(1);//product payments
        //print_r($product_payment_details);exit;
        $ads_payment_details = Payments::select_all_payments(2);//Ads payments
        
        $order_payment_details = Payments::select_all_payments(3);//Orders payments
        
        $quote_payment_details = Payments::select_all_payments(4);//Quote payments
        
        echo $this->render('//admin/payments/index',array('product_payment_details' => $product_payment_details,
                                                          'ads_payment_details' => $ads_payment_details,
                                                          'order_payment_details' => $order_payment_details,
                                                          'quote_payment_details' => $quote_payment_details));
    }
    ...
    public function actionViewpayment($id){
        ...
        $comments = Comments::get_comments_by_options($id,3); //params will by product id and type
        
        echo $this->render('//admin/payments/viewpayment', array('payment' => $payment_details,'comments' => $comments));
    }
    ...
    /***************** REQUEST PAYMENT ads,products START ******************/
    ...
    public function actionPaymentrequest()
    {
        ...
        if(!empty($_POST))
        {
            if($_POST['payment_type'] == 1) //1=product
            {
                $payment_for = $_POST['product_id'];
                $payment_details = Payments::get_payment_details_by_type($_POST['payment_type'],$_POST['product_id']);
            }
            else if($_POST['payment_type'] == 2) //1=product
            {
                $payment_for = $_POST['ad_id'];
                $payment_details = Payments::get_payment_details_by_type($_POST['payment_type'],$_POST['ad_id']);
            }
            echo $this->render('//admin/paymentrequest/index', array('payment_details' => $payment_details,'payment_type' => $_POST['payment_type'],'payment_for' => $payment_for));
        }
        else
        {
            return $this->redirect(['admin/index'])->send();
    ...
    public function actionGeneratepaymentlink()
    {
        ...
        if(!empty($_POST))
        {
            $payment_link = Payments::generate_payment_request_link($_POST);
            //if link generation success
            if($payment_link['status'] == 200)
            {
                Yii::$app->session->setFlash('success', $payment_link['message']);
            }
            
            //if link generation fails
            else if($payment_link['status'] == 400)
            {
                Yii::$app->session->setFlash('error', $payment_link['message']);
            }
            
            if($_POST['payment_type'] == 1)//1=product
                return $this->redirect(['admin/viewproduct/'.$_POST['payment_for']])->send();
            else if($_POST['payment_type'] == 2)//1=Ads
                return $this->redirect(['admin/viewad/'.$_POST['payment_for']])->send();
        }
        else
        {
            return $this->redirect(['admin/index'])->send();
    ...
    /***************** REQUEST PAYMENT ads,products END ******************/
    
    public function actionOrderspaymentrequest()
    {
        if (Yii::$app->user->isGuest) {
            return $this->redirect(['admin/index'])->send();
        }
        if(!empty($_POST))
        {
            if($_POST['payment_type'] == 3) //3=product orders
            {
                $payment_details['amount_actual'] = 0;
                $payment_details['amount_paid'] = 0;
                $check_payment_exist = Payments::get_payment_exists($_POST['payment_type'],$_POST['order_id']);
                if(count($check_payment_exist) > 0)
                {
                    $payment_details['amount_actual'] = $check_payment_exist[0]['amount_actual'];
                    $payment_details['amount_paid'] = $check_payment_exist[0]['amount_paid'];
                }
                
                $payment_for = $_POST['order_id'];
                $payment_details['user_id'] = $_POST['user_id'];
                $payment_details['manual_order_code'] = $_POST['manual_order_code'];
                $payment_details['order_id'] = $_POST['order_id'];
                $payment_details['order_type'] = $_POST['order_type'];
                $payment_details['package_type'] = $_POST['package_type'];
                echo $this->render('//admin/paymentrequest/orders', array('payment_details' => $payment_details,'payment_type' => $_POST['payment_type'],'payment_for' => $payment_for));
            }
            if($_POST['payment_type'] == 4) //4=getquote
            {
                $payment_details['amount_actual'] = 0;
                $payment_details['amount_paid'] = 0;
                $check_payment_exist = Payments::get_payment_exists($_POST['payment_type'],$_POST['quotation_id']);
                if(count($check_payment_exist) > 0)
                {
                    $payment_details['amount_actual'] = $check_payment_exist[0]['amount_actual'];
                    $payment_details['amount_paid'] = $check_payment_exist[0]['amount_paid'];
                ...
                $payment_for = $_POST['quotation_id'];
                $payment_details['user_name'] = $_POST['user_name'];
                $payment_details['user_email'] = $_POST['user_email'];
                $payment_details['user_phone'] = $_POST['user_phone'];
                $payment_details['package_type'] = $_POST['package_type'];
                echo $this->render('//admin/paymentrequest/quote', array('payment_details' => $payment_details,'payment_type' => $_POST['payment_type'],'payment_for' => $payment_for));
            }
        }
    }
    
    public function actionGenerateorderpaymentlink()
    {
        if (Yii::$app->user->isGuest) {
            return $this->redirect(['admin/index'])->send();
        }
        if(!empty($_POST))
        {
            if($_POST['payment_type'] == 3) //1=product order (check availability)
            {
                $check_payment_exist = Payments::get_payment_exists($_POST['payment_type'],$_POST['payment_for']);
                if(count($check_payment_exist) == 0)
                {
                    $transaction_data['payment_type'] = $_POST['payment_type']; //3= Product order,4=getquote
                    $transaction_data['payment_for'] = $_POST['payment_for'];

                    //generator transaction order id
                    $payment_type = "P"; //P-payment R-refund
                    if($_POST['order_type'] == 0)
                        $payment_module = "H"; //H-Hire
                    else if($_POST['order_type'] == 1)
                        $payment_module = "Y"; //Y-Buy
                    else if($_POST['order_type'] == 2)
                        $payment_module = "Z"; //Z-Both

                    $package_type = $_POST['package_type'];//product package type

                    $primary_key = $_POST['payment_for'];
                    ...
                    $transaction_data['amount_actual'] = $_POST['due_amount'];
                    ...
                    $before_order_id = $transaction_data['before_order_id'];
                }
                else{
                    $before_order_id = $check_payment_exist[0]['before_order_id'];
                }
                
            }
            
            $_POST['before_order_id'] = $before_order_id;
            ...
            if($_POST['payment_type'] == 3)//3=product order
                return $this->redirect(['admin/vieworder/'.$_POST['payment_for']])->send();
            else if($_POST['payment_type'] == 4)//4=getquote
                return $this->redirect(['admin/viewquote/'.$_POST['payment_for']])->send();
        }
        else
        {
            return $this->redirect(['admin/index'])->send();
    ...
    public function actionGeneratequotepaymentlink()
    {
        if (Yii::$app->user->isGuest) {
            return $this->redirect(['admin/index'])->send();
        }
        if(!empty($_POST))
        {
            ...
            if(count($check_payment_exist) == 0)
            {
                ...
                $transaction_data['payment_status'] = 'Not Paid';
                $transaction_data['tracking_id'] = $tracking_id;
                $transaction_data['billing_name'] = $_POST['owner_name'];
                $transaction_data['billing_phone'] = $_POST['owner_phone'];
                $transaction_data['billing_email'] = $_POST['owner_email'];
                $transaction_data['billing_comments'] = '';
                $transaction_data['billing_time'] = date('Y-m-d H:i:s');
                $transaction_data['user_id'] = 0;
    ...
    /***************** REQUEST PAYMENT orders,getquote START ******************/
    
    
    
    /***************** REQUEST PAYMENT orders,getquote END ******************/
    
    
    /*public static function actionInsertproductpayments()
    {
        
        $query = new Query;
        $all_products = $query->select(['core_products.*'])->from('core_products')->all();
        foreach($all_products as $indiv_product)
        {
            $userId = $indiv_product['user_id'];
            $userbillingDetails = User::get_user_details_by_id($userId);
            
            $transaction_data['payment_type'] = 1; //1= Products,2=Advt.
            $transaction_data['payment_for'] = $indiv_product['product_id'];

            //generator transaction order id
            $payment_type = "P"; //P-payment R-refund
            if($indiv_product['product_type'] == 0)
                $payment_module = "S"; //S-Supply
            else if($indiv_product['product_type'] == 1)
                $payment_module = "X"; //X-Sale
            else if($indiv_product['product_type'] == 2)
                $payment_module = "B"; //B-Both

            $package_type = $indiv_product['package_type'];//Selected package type

            $primary_key = $indiv_product['product_id'];

            $category_id = $indiv_product['category_id'];
            $capacity = explode(' ',$indiv_product['capacity']);//substr($indiv_product['capacity'], 0, strrpos($indiv_product['capacity'], ' '));
            $capacity = $capacity[0];
            $query = new Query;
            ...
            $transaction_data['billing_email'] = $userbillingDetails['email'];
            $transaction_data['billing_comments'] = '';
            $transaction_data['billing_time'] = $indiv_product['date_created'];
            $transaction_data['user_id'] = $indiv_product['user_id'];

            $query = new Query;
            $count = $query->select('COUNT(*) as count')->from('core_payments')->where("before_order_id = '$transaction_order_id'")->All();
            if($count[0]['count']>0)
            {
                $update_payment['payment_type']= 1; //1=product,2=Advertisements
                $update_payment['payment_for']= $indiv_product['product_id'];
                Yii::$app->db->createCommand()->update('core_payments', $update_payment,"before_order_id = '$transaction_order_id'")->execute();
            }
            else
            {
                $newtransaction = Payments::insert_new_transaction($transaction_data);
    ...
    public static function actionInsertadpayments()
    {
        
        $query = new Query;
        $all_ads = $query->select(['core_post_ads.*'])->from('core_post_ads')->where("ad_expire = '2017-10-31 23:59:59'")->all();
        foreach($all_ads as $indiv_ad)
        {
            $userId = $indiv_ad['user_id'];
            $userbillingDetails = User::get_user_details_by_id($userId);
            
            $transaction_data['payment_type'] = 2; //1= Products,2=Advt.
            $transaction_data['payment_for'] = $indiv_ad['ad_id'];

            //generator transaction order id
            $payment_type = "P"; //P-payment R-refund
            $payment_module = "A";

            $package_type = 4;//1 year package
            $advt_primary_key = $indiv_ad['ad_id'];
            $amount_actual = 400000;
            ...
            $transaction_data['billing_email'] = $userbillingDetails['email'];
            $transaction_data['billing_comments'] = '';
            $transaction_data['billing_time'] = $indiv_ad['date_created'];
            $transaction_data['user_id'] = $indiv_ad['user_id'];

            $query = new Query;
            $count = $query->select('COUNT(*) as count')->from('core_payments')->where("before_order_id = '$transaction_order_id'")->All();
            if($count[0]['count']>0)
            {
                $update_payment['payment_type']= 2; //1=product,2=Advertisements
                $update_payment['payment_for']= $indiv_ad['ad_id'];
                Yii::$app->db->createCommand()->update('core_payments', $update_payment,"before_order_id = '$transaction_order_id'")->execute();
            }
            else
            {
    ...
    public static function actionUpdateadpaymentprimary()
    {
        //echo hexdec(substr('DEI17PA100018',8));exit;
        $query = new Query;
        $all_ad_payments = $query->select(['core_payments.*'])->from('core_payments')->where("payment_type = 2")->andWhere("payment_for = 0")->all();
        foreach($all_ad_payments as $ad_payment)
        {
            $payment_order_id = $ad_payment['before_order_id'];
            $primarykey = hexdec(substr($payment_order_id,8));
            $update_payment['payment_for']= $primarykey;
            Yii::$app->db->createCommand()->update('core_payments', $update_payment,"before_order_id = '$payment_order_id'")->execute();
