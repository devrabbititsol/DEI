...
class LoginForm extends Model
{
    ...
    public function user_login($data)
    {
        ...
        if($count[0]['count']>0)
        {
            ...
            $userstatus = $query->select('*')->from('core_users')->where("email = '$email'")->andWhere("password = '$password'")->one();
            if($userstatus['user_status'] == 2) //active user
            {
                ...
                $employeerole = $query->select('*')->from('core_user_roles')->where("role_id = ".$userstatus['user_type'])->one();
                $session->set('role',$employeerole);
            }
    ...
    public function admin_login($data)
    {
        ...
        {
            //check for user status
            $userstatus = $query->select('*')->from('core_users')->where("email = '$email'")->andWhere("password = '$password'")->andWhere("user_type != 1")->one();
            if($userstatus['user_status'] == 2) //active user
            {
                //login user by email id
                Yii::$app->user->login(User::findByUseremail($email));
                ...
                $result['status'] = $userstatus['user_status'];
                $result['login'] = 'YES';
