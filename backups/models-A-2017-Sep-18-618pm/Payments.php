...
class Payments extends Model
{
    ...
    public static function update_transaction($transaction_data)
    {
        try{
            ...
            Yii::$app->db->createCommand()->update('core_payments', ['order_status'=>"$order_status",'failure_message' => "$failure_message",'bank_reference_number' => "$bank_ref_no",'amount_paid' => "$amount",'payment_status' => "$payment_status"], "tracking_id = '$tracking_id'", "response = '$response'")->execute();
            return true;
        } catch (ErrorException $ex) {
