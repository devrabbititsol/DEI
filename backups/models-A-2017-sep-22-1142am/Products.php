...
class Products extends Model
{
    ...
    public static function select_product_count_by_category_id($category_id=null)
    {
        try {
            ...
            if($category_id != null)
                ...
                $count = $query->select('COUNT(*) as count')->from('core_products')->where("product_status = 1")->andWhere(['>=','core_products.product_expires_on',date("Y-m-d H:i:s")])->All();
    ...
    public static function insert_new_product_details($data)
    {
        try{
            ...
            $insertdata['product_status'] = 2; //update this when control panel complete
            ...
            $subject="Digital Equipments India | Registration Of Equipment";
    ...
    {
        try{
            Yii::$app->db->createCommand()->update('core_products', ['product_status' => 'product_status'+1], "product_id = '$product_id'")->execute();
            return true;
        } catch (ErrorException $ex) {
