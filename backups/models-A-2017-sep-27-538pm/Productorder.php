...
class Productorder extends Model
{
    ...
    {
        $query = new Query;
        $orders = $query->select(['core_orders.*','core_product_categories.category_name','core_users.user_name as employee_name'/*,'core_product_sub_categories.sub_category_name', 'core_product_models.model_name'*/])
                        ->from('core_orders')
    ...
    public static function get_orders_by_userid($userid)
    {
        ...
        return $orders = $query->select(['core_orders.*','core_product_categories.category_name','core_users.user_name as employee_name'/*,'core_product_sub_categories.sub_category_name', 'core_product_models.model_name'*/])
                        ->from('core_orders')
                        ->innerJoin('core_products', 'core_orders.product_id=core_products.product_id')
                        ->innerJoin('core_product_categories', 'core_product_categories.category_id=core_products.category_id')
                        ->leftJoin('core_users', 'core_orders.employee_id=core_users.user_id')
    ...
    public static function update_order_status($order_id,$status)
    {
         try{
            $userId = Yii::$app->user->id;
            $date_updated = date('Y-m-d H:i:s');
            Yii::$app->db->createCommand()->update('core_orders', ['order_status' => $status,'updated_by' => $userId,'date_updated' => $date_updated], "order_id = '$order_id'")->execute();
            return true;
        } catch (ErrorException $ex) {
