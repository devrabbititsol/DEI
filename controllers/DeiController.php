...
class DeiController extends Controller
{
    ...
    public function beforeAction($action) {
        ...
        if($action->id == 'uploadproductimages' ||  $action->id == 'uploadadvtimages' || $action->id == 'uploadproductloadcharts' || $action->id == 'paynowcancel'|| $action->id == 'paynowsuccess' || $action->id == 'productpaymentsuccess' || $action->id == 'productpaymentcancel' || $action->id == 'productpaymentcancel' || $action->id == 'facebookadpaymentsuccess' || $action->id == 'facebookadpaymentcancel' || $action->id == 'makepaymentrequest'|| $action->id == 'paymentrequestcancel'|| $action->id == 'paymentrequestsuccess') {
    ...
    public function actionLogout()
    {
        Yii::$app->user->logout();
        //return $this->goHome();
        return $this->redirect(Yii::$app->request->referrer);//redirect to last viewing page.
    }
        ...
        try{
            ...
            if (!isset($_POST['repeat'] )) 
            {
                ...
                else
                {
                    ...
                    Payments::insert_new_transaction($transaction_data);
                    
                    /*Payments table inserts for PROMOTIONAL PRODUCT END*/
    ...
    public function actionFacebookad()
    {
        ...
        try{
            ...
            /*Payment Insert for PROMOTIAL ADS START*/
            else if(count($response['err'])==0 && $_REQUEST['advt_package'] == 0)
            {
                $userId = Yii::$app->user->id;
                $userbillingDetails = User::get_user_details_by_id($userId);

                $transaction_data['payment_type'] = 2; //1= Products,2=Advt.
                $transaction_data['payment_for'] = $response['ad_id'];

                //generator transaction order id
                $payment_type = "P"; //P-payment R-refund
                $payment_module = "A";

                $package_type = 1;//1 month package
                $advt_primary_key = $response['ad_id'];
                $amount_actual = 50000;
            ...
            /*Payment Insert for PROMOTIAL ADS END*/
    ...
    /*public function actionCopytest()
    {
	//echo "<img src='uploads/fb_79701506515515.jpg'>";
        if(copy('uploads/3581496310148.jpg', 'temp/fb_79701506515515.jpg'))
            echo "copied";
        else
            echo "Not copied";
     }*/
    //action to display paynow cart page
    public function actionPaynow()
    {
        if (Yii::$app->user->isGuest || empty($_POST)) {
            return $this->goHome();
        }
        $payment_details = Payments::get_payment_by_id($_POST['payment_id']);
        $userId = Yii::$app->user->id;
        $userbillingDetails = User::get_user_details_by_id($userId);
        
        $redirectUrl = Yii::$app->params['SITE_URL'].'paynowsuccess';
        $cancelUrl = Yii::$app->params['SITE_URL'].'paynowcancel';
        
        $current_time_stamp = strtotime(date("Y-m-d H:i:s"));
        $order_id = $payment_details['before_order_id'].'#'.$current_time_stamp;
        $tracking_id = $current_time_stamp;
        
        $params = [
                    'tid' => $tracking_id,
                    'order_id' => $order_id,
                    'amount_actual' => $payment_details['amount_actual'],
        ...
        echo $this->render('//paynowcart', array('params' => $params));
        
        
    }
    
    public function actionPaynowcancel()
    {
        if (Yii::$app->user->isGuest) {
            return $this->goHome();
        }
        echo $this->render('//paynowcancelmessage');
        
    }
    public function actionPaynowsuccess()
    {
        
        if (Yii::$app->user->isGuest || empty($_REQUEST)) {
            return $this->goHome();
        }
        
        $response = Yii::$app->request->bodyParams;
        $transaction_data = Payments::decrypt_response($response);
        Payments::update_paynow_transaction($transaction_data);
        
        return $this->render('//paynowsuccessmessage', array('transaction_data' => $transaction_data));
        
    }
    
    //action to display paynow cart page
    public function actionMakepaymentrequest($id)
    {
        /*if (Yii::$app->user->isGuest) {
            $session = Yii::$app->session;
            $session->set('REDIRECT_URI', Yii::$app->params['SITE_URL'].'makepaymentrequest?id='.$id);
            return $this->redirect(['login']);
        }*/
        
        $payment_link_details = Payments::check_payment_link_expire($id);
        
        if(count($payment_link_details) > 0)
        {
            $payment_link_details = $payment_link_details[0];
            //$userId = Yii::$app->user->id;
            //$userbillingDetails = User::get_user_details_by_id($userId);

            $redirectUrl = Yii::$app->params['SITE_URL'].'paymentrequestsuccess';
            $cancelUrl = Yii::$app->params['SITE_URL'].'paymentrequestcancel';

            $current_time_stamp = strtotime(date("Y-m-d H:i:s"));
            $order_id = $payment_link_details['before_order_id'].'#'.$current_time_stamp;
            $tracking_id = $current_time_stamp;

            $params = [
                        'tid' => $tracking_id,
                        'order_id' => $order_id,
                        'amount' => $payment_link_details['due_amount'],
                        'currency' => 'INR',
                        'redirect_url' => $redirectUrl,
                        'cancel_url' => $cancelUrl,
                        'language' => 'EN',
            ...
            echo $this->render('//paymentrequestcart', array('params' => $params));
        }
        else
        {
            return $this->goHome();
    ...
    public function actionPaymentrequestcancel()
    {
        
        echo $this->render('//paymentrequestcancelmessage');
        
    }
    public function actionPaymentrequestsuccess()
    {
        ...
        return $this->render('//paymentrequestsuccessmessage', array('transaction_data' => $transaction_data));
        
    }
    
    public function actionFranchisepartner()
    {
        
        return $this->render('//franchisepartner');
        
    }
    
    //Dynamic filtering of products
    public function actionGetproductsdynamiclist()
    {
        ...
        foreach($products as $index=>$product) { 
            ...
            $selected_product_ids = explode(',',$_REQUEST['search_selected_product_ids']);
            ...
            $out .= '<input '
                    ...
                    . 'onclick="updateproductids('.$product['product_id'].','.(($product['price_type'] == 1)?"'Days'":"'Months'").')"'
                    . ((Yii::$app->user->id == $product['user_id'])? "disabled style='opacity:0'":"").'><label for="styled-checkbox-1" class="checkc"></label></form>';
            $out .= '<div class="itemdtls">';
            $out .= '<div class="itemimg"><img src="'.$product['image_url'].'" alt="" width="93px" height="85px"></div>';
            $out .= '<div class="itemcon">';
            $out .= '<h4>'.$product['equipment_title'].'<span style="display:none;">'.$product['manual_product_code'].'</span></h4><ul class="list-inline">';
            
            if($_REQUEST['product_type']=='hire'){  
                if($product['hire_price'] == -1.00){ 
                    $out .= '<li><i class="fa fa-rupee"></i>PRICE ON REQUEST/'.(($product['price_type'] == 1)?"Daily":"Monthly").'</li>';   
                }
                else{
                    $out .= '<li><i class="fa fa-rupee"></i>'.$product['hire_price'].'/'.(($product['price_type'] == 1)?"Daily":"Monthly").'</li>';
                }
            }
            else 
            {
                ...
                {
                    $out .= '<li><i class="fa fa-rupee"></i>PRICE ON REQUEST</li>';
                }
                else{
                    $out .= '<li><i class="fa fa-rupee"></i>'.$product['sale_price'].'</li>';
                }
            }
            $out .= '<li><strong>Capacity:</strong>'.$product['capacity'].'</li>';
            ...
            $out .= '<li><strong><i class="fa fa-map-marker"></i>Location</strong>'.$currentlocation.'</li></ul></div></div>';
            $out .= '<div class="itembtn"><a href="javascript:void(0);" class="btn btn-border-bei" onclick="getProductdata('.$product['product_id'].');">View</a></div>';
            $out .= '</div></td></tr>';
    ...
    {
        try{
            $products=Products::select_products_by_search($_REQUEST);
            $productcategories = Productcategory::find()->orderBy(['display_order' => SORT_ASC])->all();
            return $this->render('//products', array('products' => $products,'productcategories' => $productcategories,'options' => $_REQUEST));
