...
class Ads extends Model
{
    ...
    public static function select_all_ads()
    {
        ...
        /*return $query->select(['core_post_ads.*','core_post_ads_images.ad_image_url'])
                    ...
                    ->all();*/
        $result = $query->select(['core_post_ads.*','core_post_ads_images.ad_image_url','core_users.company_name','core_post_ads_images.ad_type'])
    ...
    public static function insert_facebook_post($data)
    {
        ...
        if($data['ad_type'] == 1) //posting images advt.
        {
            //insert images related to that ad post.
            ...
            foreach ($all_files as $index => $image) {
                ...
                $insertimage['ad_type'] = $data['ad_type'];
        ...
        else if($data['ad_type'] == 2) //posting Video advt.
        {
            $all_files = $data['video_link'];
            foreach ($all_files as $index => $video_link) {
                if($video_link != '')
                {
                    $insertimage = array();
                    $insertimage['ad_id'] = $ad_id;
                    $insertimage['ad_image_name'] = $video_link;
                    $insertimage['ad_image_url'] = $video_link;
                    $insertimage['ad_image_expire'] = $ad_expire;
                    $insertimage['ad_image_status'] = 0; //Initally Inactive
                    $insertimage['updated_by'] = Yii::$app->user->id;
                    $insertimage['date_updated'] = date('Y-m-d H:i:s');
                    $insertimage['ad_type'] = $data['ad_type'];
                    ...
                    if ($data['advt_package'] == 0)//0=promotional
                    {
                        ...
                        //post Video to facebook wall
                        try {

                            //$fbdata['url'] = $fbpostimg;
                            $fbdata['link'] = $video_link;
                            $fbdata['source'] = $video_link;
                            $fbdata['message'] = $data['title'] . ' ' . $data['weblink'];
                            $fbdata['access_token'] = $fb_access_token;
                            $post_url = 'https://graph.facebook.com/' . $fb_page_id . '/feed';
                            $ch = curl_init();
                            curl_setopt($ch, CURLOPT_URL, $post_url);
                            curl_setopt($ch, CURLOPT_POST, 1);
                            curl_setopt($ch, CURLOPT_POSTFIELDS, $fbdata);
    ...
    public static function post_advt_on_facebook_wall($ad_id)
    {
        ...
        $fbposts = $query->select(['core_post_ads_images.ad_image_url','core_post_ads.ad_title','core_post_ads.ad_weblink'])
                    ->from('core_post_ads_images')
                    ->innerJoin('core_post_ads', 'core_post_ads.ad_id=core_post_ads_images.ad_id')
        ...
        {
            if($post['ad_type'] == 1)//check if ad is image
            {
                //post image to facebook wall
                try{
            ...
            else if($post['ad_type'] == 2)//check if ad is Video
            {
                //post Video to facebook wall
                try{

                    //$fbdata['url'] = $fbpostimg;
                    $fbdata['link'] = $post['ad_image_url'];
                    $fbdata['source'] = $post['ad_image_url'];
                    $fbdata['message'] = $post['ad_title'].' '. $post['ad_weblink'];
                    $fbdata['access_token'] = $fb_access_token;
                    $post_url = 'https://graph.facebook.com/'.$fb_page_id.'/feed';
                    $ch = curl_init();
