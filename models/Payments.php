...
class Payments extends Model
{
    ...
    public static function update_transaction($transaction_data)
    {
        try{
            ...
            $already_paid_amount = $query->select(['core_payments.amount_paid','core_payments.amount_actual','core_payments.payment_status','core_payments.payment_id','core_payments.user_id'])
                                    ...
                                    ->one();
            //get amount paid without gst
            $amount_paid = $amount/((Yii::$app->params['GST']/100)+1);
            if($order_status == 'Success')
            {
                if($amount_paid >= $already_paid_amount['amount_actual'])
                {
                    $payment_status = 'Paid';
                }
                else
            ...
            else
            {
                ...
                $amount_paid = $already_paid_amount['amount_paid'];
            }
            
            $response = json_encode($transaction_data);
            Yii::$app->db->createCommand()->update('core_payments', ['order_status'=>"$order_status",'failure_message' => "$failure_message",'bank_reference_number' => "$bank_ref_no",'amount_paid' => "$amount_paid",'payment_status' => "$payment_status",'response' => "$response"], "before_order_id = '$order_id'")->execute();
            
            $transaction_history = array();
            $transaction_history['payment_id'] = $already_paid_amount['payment_id'];
            $transaction_history['amount'] = $amount/((Yii::$app->params['GST']/100)+1);//get amount without gst
            $transaction_history['gst_amount'] = $amount-($amount/((Yii::$app->params['GST']/100)+1)); //get gst amount from total
    ...
    public static function update_paynow_transaction($transaction_data)
    {
        try{
            ...
            if($order_status == 'Success')
            {
                $amount_paid = $already_paid_amount['amount_paid'] + $amount_paid;
                if($amount_paid >= $already_paid_amount['amount_actual'])
                {
                    $payment_status = 'Paid';
                }
    ...
    public static function generate_payment_request_link($data)
    {
        try{
            ...
            $mail_info = array();
            if($data['payment_type'] != 4) //4=getquote
            {
                ...
                $toemail = $product_owner_details->email;
            }
            else{
                $mail_info['owner_email'] = $data['owner_email'];
                $mail_info['owner_name'] = $data['owner_name'];
                $toemail = $data['owner_email'];
            }
            ...
            {
                $mail_info['manual_order_code'] = $data['manual_order_code'];
                
                $subject="Big Equipments India | Order amount request";
                $message = Mail_settings::get_payment_request_for_order($mail_info);
            }
            else if($data['payment_type'] == 4)
            {
                
                $subject="Big Equipments India | Getquote amount request";
                $message = Mail_settings::get_payment_request_for_quote($mail_info);
            }
            
            Mail_settings::send_email_notification($toemail,$subject,$message);
    ...
    public static function get_payment_exists($payment_type,$payment_for)
    {
        $query = new Query;
        return $query->select(['core_payments.*'])
                ->from('core_payments')
                ->where("core_payments.payment_type = '$payment_type'")
                ->andWhere("core_payments.payment_for = '$payment_for'")
                ->all();
