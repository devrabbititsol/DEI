...
class Products extends Model
{
                ...
                $pending_products_count = $query->select('COUNT(*) as count')->from('core_products')
                    ...
                    if($role_details['role_id'] != 2 && $role_details['role_id'] != 3 && $role_details['role_id'] != 8)//admin or superadmin or dataoperator
                    {
                        ...
                        if($role_details['role_id'] == 4)//zonal manager
                        {
                            ...
                            $pending_products_count = $pending_products_count->andWhere(['core_products.employee_id' => $filteredemployees]); 
                        ...
                        {
                            $pending_products_count = $pending_products_count->andWhere("core_products.employee_id = $userId"); 
                        }

                    }
                               //->andWhere(['>=','core_products.product_expires_on',date("Y-m-d H:i:s")])
                $pending_products_count = $pending_products_count->All();//pending products count 
                ...
                $count = $query->select('COUNT(*) as count')->from('core_products');
                    ...
                    if($role_details['role_id'] != 2 && $role_details['role_id'] != 3 && $role_details['role_id'] != 8)//admin or superadmin or dataoperator
                    {
                        ...
                        if($role_details['role_id'] == 4)//zonal manager
                        {
                            $underthiszoneusers = Yii::$app->db->createCommand("SELECT user_x_roles.user_id from user_x_roles where user_x_roles.user_zone_id IN (SELECT user_zone_id from user_x_roles where user_x_roles.user_id = $userId) GROUP BY user_id")->queryAll();
                            foreach($underthiszoneusers as $employee_id) $filteredemployees[] = $employee_id['user_id'];
                            $count = $count->where(['core_products.employee_id' => $filteredemployees]); 
                        }else if($role_details['role_id'] == 5)//state manager
                        {
                            $underthiszoneusers = Yii::$app->db->createCommand("SELECT user_x_roles.user_id from user_x_roles where user_x_roles.user_state_id IN (SELECT user_state_id from user_x_roles where user_x_roles.user_id = $userId) GROUP BY user_id")->queryAll();
                            foreach($underthiszoneusers as $employee_id) $filteredemployees[] = $employee_id['user_id'];
                        ...
                        {
                            $count = $count->where("core_products.employee_id = $userId"); 
                        }
    ...
    public static function select_all_our_services()
    {
        ...
        $services = $query->select(['core_products.*','core_product_categories.category_name','core_product_sub_categories.sub_category_name', 'core_product_models.model_name','core_product_categories.metric AS metric','core_product_images.*'])
                    ...
                    ->andWhere("core_product_images.image_status = 1")
                    ->andWhere("core_product_images.image_type = 1")
                    ->andWhere(['>=','core_products.product_expires_on',date("Y-m-d H:i:s")])
                    ->all();
        return $services;
    }
    ...
    public static function select_products_by_options($data)
    {
        ...
        $products = $query->select(['core_products.*','core_product_images.*'])
                    ->from('core_products')
                    ->innerJoin('core_product_images', 'core_product_images.product_id=core_products.product_id')
                    ->innerJoin('core_product_locations', 'core_product_locations.product_id=core_products.product_id');
        ...
        if(@$_REQUEST['global_search'] != '')
        {
            $products = $products->andFilterWhere([
                            'or',
                            ['LIKE', 'core_products.equipment_title', $_REQUEST['global_search']],
                            ['LIKE', 'core_products.manual_product_code', $_REQUEST['global_search']],
        ...
        //filter by sort
        if(@$_REQUEST['sort_by'] == 'location')
        {
            $currentlocation = Generalsettings::get_country_name_by_ip();
            $lon = $currentlocation['longitude'];
            $lat = $currentlocation['latitude'];
            $products = $products->orderBy(["POW((core_product_locations.longitude-$lon),2) + POW((core_product_locations.latitude-$lat),2)" => SORT_ASC]);
        }
        else
        {
    ...
    public static function select_product_by_id($data)
    {
        ...
        $productdata = $productquery->select(['core_products.*','core_product_categories.category_name','core_product_sub_categories.sub_category_name', 'core_product_models.model_name','core_product_categories.metric','core_product_images.*'])
                    ...
                    ->andWhere("core_product_images.image_type = 1")
                    ->all();
        //return  $productquery->createCommand()->getRawSql();;
    ...
    public static function get_products($limit = null,$product_type = null)
    {
        ...
        if($role_details['role_id'] != 2 && $role_details['role_id'] != 3 && $role_details['role_id'] != 8)//admin or superadmin or dataoperator
        {
            ...
            {
                $products = $products->andWhere("core_products.employee_id = $userId"); 
            }
    ...
    public static function update_product_details($data)
    {
        try{
            ...
            $insertdata['updated_by'] = Yii::$app->user->id;
            $insertdata['date_updated'] = date('Y-m-d H:i:s');
            $insertdata['product_expires_on'] = date('Y-m-d', strtotime($insertdata['product_expires_on']))." 23:59:59";
    ...
    //function to display results to user while searching for products
    public static function select_products_by_search($data)
    {
        ...
        if(@$_REQUEST['global_search'] != '')
        {
            $products = $products->andFilterWhere([
                            ...
                            ['LIKE', 'core_products.capacity', $_REQUEST['global_search']],
                            ['LIKE', 'core_products.current_location', $_REQUEST['global_search']],
                            ['LIKE', 'core_product_categories.category_name', $_REQUEST['global_search']],
                            ['LIKE', 'core_product_sub_categories.sub_category_name', $_REQUEST['global_search']],
