...
//if($cookies->has('new_selected_product_ids'))
    $selected_product_ids = explode(',',$cookies->getValue('selected_product_ids'));
...
<?php if(@$options['global_search'] != ''){ ?>
<div class="col-md-8 col-md-offset-5"><h4 class="text-danger"> You are searching for <strong><?php echo @$options['global_search']; ?></strong></h4></div>
<?php } ?>
<div class="container">
    <div class="row">
        <div class="col-md-3">
            ...
            <form class="crane_left_filters" action="products" method="post" id="searchform">
                ...
                <input type="hidden" name="search_selected_product_ids" id="search_selected_product_ids" value="<?php echo $cookies->getValue('selected_product_ids'); ?>">
                <input type="hidden" name="sort_by" id="sort_by" value="new">
                <input type="hidden" name="global_search" id="global_search" value="<?php echo @$options['global_search']; ?>">
                <div class="head-1">
                    <h1>Filters</h1>
                </div>
                <?php 
                ...
                <button type="button" class="btn btn-bei" onclick="return resetsearch();">Reset</button>
            </form>
        </div>
        <div class="col-md-9">
            <div class="crane_right_filters">
                ...
                <div class="text-right">
                    ...
                    <form class="form-inline">
                        ...
                        <div class="form-group pull-left pt-10">
                            <label>Sort by</label>
                            <select class="form-control" onchange="getDatausingsortby()" id="sortby">
                    ...
                    <div class="itemdtls">
                        ...
                        <div class="itemcon">
                            ...
                            <ul class="list-inline">
                                ...
                                <li><i class="fa fa-rupee"></i>PRICE ON REQUEST/<?php if($product['price_type'] == 1) echo "Daily"; else echo "Monthly"; ?></li>
                                <?php } else{ ?>
                                ...
                                <?php } 
                                 ...
                                 { ?>
                                <li><i class="fa fa-rupee"></i>PRICE ON REQUEST</li>
                                 <?php }else{  ?>
                                ...
                                <span style="display: none;"><?php echo $product['current_location']; ?></span>
                            </ul>
                        </div>
                    </div>
            ...
            <div class="tab-content">
                ...
                <?php } ?>
                    <div class="tab-pane <?php if(!Yii::$app->user->isGuest) { echo "active"; }?>" role="tabpanel" id="step3">
                        ...
                        <input type="hidden" name="product_id" id="product_id" value="<?php echo implode(',',$selected_product_ids); ?>">
...
{
    $.ajax({
        url: "getproductsdynamiclist",
        data : $('#searchform').serialize(),
        dataType: 'json',
        success: function(data){
            $('#productslist').DataTable().destroy();
            $('#productslist tbody').html(data.out);
            $('#productslist').DataTable({
                ...
                "fnDrawCallback": function (oSettings) {
                 ...
                 "displayStart":parseInt(current_page)
            });
        }
    });
}
...
$(document).ready(function(){
    if($.cookie('selected_product_ids')!='')
    {
        <?php if(!empty($selected_product_ids)) { ?>
            $(".checkavailablebutton").removeClass('disabled');  
            multipleCheckavailability();
        <?php } ?>            
                    
        
    }
    current_page = 0;
    if($.cookie('selected_product_ids')!='')//if cookie is set
    {
        $('#productslist tr').filter(':has(:checkbox:checked)').each(function() {
            current_page = Math.floor((this.id)/10)*10;
        });
    }
    
    productlistdt = $('#productslist').DataTable({
        "aaSorting": [],
        "fnDrawCallback": function (oSettings) {
            $("#productslist_filter input[type='search']").addClass("form-control datatable-search");
...
function order_now(product_id,type)
{
    ...
    $.cookie('selected_product_ids', product_id);
    ...
    if($("#addorder").valid())
    {
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                $(".orderloading").hide();
                $.cookie('selected_product_ids', '');//uncheck selected products from cookie
            },
...
$('#order_details').on('hidden.bs.modal', function (e) {
   <?php if(isset($_GET['product_id'])){ ?>
           window.location.href="<?= Yii::$app->params['SITE_URL'] ?>";
   <?php }else{ ?>
   $('#addorder')[0].reset();
   <?php } ?>
...
function updateproductids(id,cur_product_price_type = null)
{
    ...
    if($("#multiavailabity"+id).is(':checked'))
    {
        ...
        $.cookie('selected_product_ids', result.join(','));
        $("#search_selected_product_ids").val(result.join(','));
